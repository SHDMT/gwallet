...
func openBadgerDb(keyDir, valueDir string) (*db, error) {
	...
	lockFilePath := filepath.Join(keyDir, lockFileName)
	...
	lockFile, err := os.OpenFile(lockFilePath, os.O_RDWR, 0x0666)
	if err != nil {
		return nil, database.NewDBError(database.ErrInOpenDB, "Failed to open lock file", err)
	}
